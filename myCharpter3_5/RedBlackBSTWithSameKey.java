...
public class RedBlackBSTWithSameKey<Value extends Comparable<Value>> {
	private Node root;
	...
	public Iterable<Value> getAll(int key)
	{ return getAll(root,key);}
	
	private Iterable<Value> getAll(Node x,int key){
		if(x==null) return null;
		int cmp = key - x.key;
		if(cmp<0) return getAll(x.left,key);
		else if(cmp>0) return getAll(x.right,key);
		else 		return (Iterable<Value>) x.val.iterator();
	}
	...
	private Node delete(Node h,int key) {
		if(key<h.key) {
			...
			if(!isRed(h.right)&&!isRed(h.right.left))
				h = moveRedRight(h);
			if(key==h.key) {
				h.val = change(h.right,min(h.right).key);
	...
	private Queue change(Node x,int key) {
		if(x==null) return null;
		int cmp = key - x.key;
		if(cmp<0) return change(x.left,key);
		else if(cmp>0) return change(x.right,key);
		else 		return x.val;
	}
