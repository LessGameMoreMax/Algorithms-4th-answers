...
public class MoveToFrontArrayST<Key extends Comparable<Key>,Value> {
	private Key[] keys;
	private Value[] vals;
	private int N;

	public MoveToFrontArrayST() {
	...
	public Value get(Key key) {
		...
		int i = rank(key);
		if(i>=N) return null;
		Key toFrontKey = keys[i];
		Value toFrontValue = vals[i];
		for(int j = i;j>0;j--) {
			keys[j] = keys[j-1];
			vals[j] = vals[j-1];
		}
	...
	MoveToFrontArrayST<String,Double> reportCard = new MoveToFrontArrayST();
